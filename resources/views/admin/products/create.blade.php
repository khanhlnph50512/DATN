...
<form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div id="primary-image-options" style="margin-bottom: 20px;"></div>
    ...
    @foreach($colors as $color)
        <label style="margin-right: 15px; cursor:pointer; display:inline-flex; align-items:center;">
            <input type="checkbox" name="colors[]" value="{{ $color->id }}" {{ is_array(old('colors')) && in_array($color->id, old('colors')) ? 'checked' : '' }}>
            <span style="display:inline-block; width:15px; height:15px; background-color: {{ $color->code ?? '#000' }}; border: 1px solid #ccc; margin-left:5px; margin-right:5px;"></span>
            {{ $color->name }}
        </label>
    @endforeach
    <br><br>
...
<script>
    ...
    function createQuantityInputs() {
        ...
        if(selectedSizes.length === 0 || selectedColors.length === 0) {
            container.innerHTML = '<p style="color:#888;">Vui lòng chọn ít nhất 1 Size và 1 Màu để nhập số lượng biến thể.</p>';
            return;
        }
