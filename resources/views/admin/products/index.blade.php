    ...
    @forelse($products as $p)
        <tr>
            <td style="text-align: center;">{{ $p->id }}</td>
             <td>
                @if($p->primaryImage)
                    <img src="{{ asset('storage/' . $p->primaryImage->image_url) }}" alt="{{ $p->name }}" width="80">
                @else
            ...
            <td style="text-align: center;">
                ...
                <form action="{{ route('admin.products.destroy', $p->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button onclick="return confirm('Bạn có chắc muốn xóa sản phẩm này?');" style="background: none; border: none; color: red; cursor: pointer;">Xóa</button>
                </form>
            </td>
        </tr>
    @empty
